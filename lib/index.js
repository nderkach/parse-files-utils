...
function initialize(config) {
  questions(config).then(function (answers) {
    ...
    return getAllFileStringObjects();
...
function getAllFileStringObjects() {
  console.log("Fetching schema...");
  return schemas.get().then(function(res){
    console.log("Fetching all objects with file strings...");
    var schemasWithFiles = onlyString(res);
    return Promise.all(schemasWithFiles.map(getObjectsWithFileStringsFromSchema));
  }).then(function(results) {
    var files = results.reduce(function(c, r) {
      return c.concat(r);
    }).filter(function(obj) {
      return obj.url !== undefined? obj.url.includes("parsetfss"): false;
    });
...
function getObjectsWithFileStringsFromSchema(schema) {
  var query = new Parse.Query(schema.className);
  query.select(schema.fields.concat('createdAt'));
  query.ascending('createdAt');
  query.limit(1000);
